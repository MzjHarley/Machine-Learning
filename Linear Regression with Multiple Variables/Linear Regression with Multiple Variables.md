...
+ Gradient descent needs to choose the learning rate $α$, iterates many steps, but the normal equation only needs one step.  
...
What happens when the matrix$~ X^TX$ is irreversible? Irreversible problems rarely occur, and even if they do, the result can be calculated normally using $np.~ linalg. ~ pinv()$.
>$np.~ linalg. ~ pinv(), ~ pseudo-inverse,can ~ find ~ the ~ inverse ~ matrix ~ of ~ singular ~ matrices ~ and ~ even ~ non-singular ~ matrices$.  
>$np.~ linalg. ~ inv(), ~  inverse,introduces ~ concepts ~ of ~ advanced ~ numerical ~ computing$.
